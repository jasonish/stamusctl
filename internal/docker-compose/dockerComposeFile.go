...
services:
  ...
  logstash:
    ...
    healthcheck:
      ...
      retries: 3
      start_period: 30s
    mem_limit: {{.LogstashMemory | default "3G"}}
    volumes:
  ...
  scirius:
    ...
    image: {{.Registry | default "ghcr.io/stamusnetworks"}}/scirius:{{.SciriusToken | default "master"}}
    restart: {{.RestartMode | default "unless-stopped"}}
    environment:
  ...
  arkime:
    ...
    image: {{.Registry | default "ghcr.io/stamusnetworks"}}/arkimeviewer:{{.ArkimeviewerVersion | default "master"}} ## Repo will need to be changed to stamusnetwork once image built
